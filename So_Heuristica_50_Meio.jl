...
function PlotaPontos(n,L,atracoes,R,H)
...
function DadosMeio(T,atracoes,Hoteis,Restaurantes)
    Random.seed!(0)
    ...
    raio=2
    θ1 = 2*pi*rand(Restaurantes,2)
    θ2 = 2*pi*rand(Restaurantes,2)

    auxR = centroX .+ raio*cos.(θ1);centroY .+ raio*sin.(θ1)
    auxH = centroX .+ raio*cos.(θ2);centroY .+ raio*sin.(θ2)
...
(n,L,a,b,c,d,e,P,Q,R,H,M,w0,n_min,n_max) = DadosMeio(T,atracoes,Hoteis,Restaurantes)

p = PlotaPontos(n,L,atracoes,R,H)
p[end]
...
for λ in [0,1]
    ...
    if λ < 0.5
        ...
        Z10 .= z1
        Z20 .= z2
        FileIO.save("50_Path0_Meio.jld2","RH",Path)
        FileIO.save("50_Ordem0_Meio.jld2","RH",Ordem)
        FileIO.save("50_Visita0_Meio.jld2","RH",Visita)
        FileIO.save("50_Horas0_Meio.jld2","RH",Horas)
    else
        GAP[2]=mean(g)
        CPU1[2]=Cputime1
        CPU2[2]=sum(tem)
        Z11 .= z1
        Z21 .= z2
        FileIO.save("50_Path1_Meio.jld2","RH",Path)
        FileIO.save("50_Ordem1_Meio.jld2","RH",Ordem)
        FileIO.save("50_Visita1_Meio.jld2","RH",Visita)
        FileIO.save("50_Horas1_Meio.jld2","RH",Horas)
    end
end
...
for λ ∈ [0.2,0.5,0.8]
    ...
    CPU1[k]=Cputime1
    CPU2[k]=sum(tem)
    FileIO.save("50_Path_Meio$compr.jld2","RH",Path)
    FileIO.save("50_Ordem_Meio$compr.jld2","RH",Ordem)
    FileIO.save("50_Visita_Meio$compr.jld2","RH",Visita)
    FileIO.save("50_Horas_Meio$compr.jld2","RH",Horas)
end
FileIO.save("50_CPU1_Meio.jld2","RH",CPU1)
FileIO.save("50_CPU2_Meio.jld2","RH",CPU2)
FileIO.save("50_GAP_Meio.jld2","RH",GAP)

#-------------------------------------------------

Path0 = FileIO.load("50_Path0_Meio.jld2","RH")
Path1 = FileIO.load("50_Path1_Meio.jld2","RH")
Path2 = FileIO.load("50_Path_Meio2.jld2","RH")
Path5 = FileIO.load("50_Path_Meio5.jld2","RH")
Path8 = FileIO.load("50_Path_Meio8.jld2","RH")

Ordem0 = FileIO.load("50_Ordem0_Meio.jld2","RH")
Ordem1 = FileIO.load("50_Ordem1_Meio.jld2","RH")
Ordem2 = FileIO.load("50_Ordem_Meio2.jld2","RH")
Ordem5 = FileIO.load("50_Ordem_Meio5.jld2","RH")
Ordem8 = FileIO.load("50_Ordem_Meio8.jld2","RH")

CPU1_Meio = FileIO.load("50_CPU1_Meio.jld2","RH")
CPU2_Meio= FileIO.load("50_CPU2_Meio.jld2","RH")
GAP_Meio = FileIO.load("50_GAP_Meio.jld2","RH")
