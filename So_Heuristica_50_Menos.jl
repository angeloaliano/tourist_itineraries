...
function Plotasolucao(n,L,X,atracoes,R,H)
    scatter([L[2,1]], [L[2,2]],size="400,400",linewidth="3",
    pointtype="fsquare", plotstyle="linespoints",
    linecolor = "'white'",
    Axes(key = :off,xlabel = :x, ylabel = :y,xrange=(0,10.5), yrange=(0,10.5)))
    p=[]
    for i in 1:n, j in 1:n
        if X[i,j] == 1
            push!(p,plot!([L[i,1],L[j,1]],[L[i,2],L[j,2]],linewidth="0.8",linecolor = "'blue'",plotstyle = "linespoints", pointtype=""))
        end
    end
...
function Dados(T,atracoes,Hoteis,Restaurantes)
    ...
    for t in 1:maximum(T)
        ...
        auxPR=[auxPR;[10;5;2;1]]
        if t<maximum(T)
            AuxH=[AuxH;auxH]
            auxTH=[auxTH;8*ones(Restaurantes)]
            auxPH=[auxPH;[1;2;5;10]]
        end
    end
    ...
    n_min=Int.(4*ones(maximum(T),1))
    n_max=Int.(8*ones(maximum(T),1))
...
function Primeira_heuristica_compromisso(TempoH,λ,a,b,c,d,e,P,Q,n_min,n_max,w0,atracoes,R,H,z10,z20,z11,z21)
    ...
    for dia=1:maximum(T)
        if dia == 1
            ...
            Kmax = maxima_ordem(Y,ind,1,n_max[1]+3)
            visitados = Visitados(n,ind,Phi,dia)
            ultima_visita = Int(round(sum(i*value(x[i,1]) for i in ind),digits=0))
            UV[dia] = ultima_visita
            W0 = W[ultima_visita]
        else
            if dia < maximum(T)
...
for λ in [0,1]
    ...
    if λ < 0.5
        ...
        Z10 .= z1
        Z20 .= z2
        FileIO.save("50_Path0_Menos.jld2","RH",Path)
        FileIO.save("50_Ordem0_Menos.jld2","RH",Ordem)
        FileIO.save("50_Visita0_Menos.jld2","RH",Visita)
        FileIO.save("50_Horas0_Menos.jld2","RH",Horas)
    else
        GAP[2]=mean(g)
        CPU1[2]=Cputime1
        CPU2[2]=sum(tem)
        Z11 .= z1
        Z21 .= z2
        FileIO.save("50_Path1_Menos.jld2","RH",Path)
        FileIO.save("50_Ordem1_Menos.jld2","RH",Ordem)
        FileIO.save("50_Visita1_Menos.jld2","RH",Visita)
        FileIO.save("50_Horas1_Menos.jld2","RH",Horas)
    end
end
...
for λ ∈ [0.2,0.5,0.8]
    ...
    CPU1[k]=Cputime1
    CPU2[k]=sum(tem)
    FileIO.save("50_Path_Menos$compr.jld2","RH",Path)
    FileIO.save("50_Ordem_Menos$compr.jld2","RH",Ordem)
    FileIO.save("50_Visita_Menos$compr.jld2","RH",Visita)
    FileIO.save("50_Horas_Menos$compr.jld2","RH",Horas)
end
FileIO.save("50_CPU1_Menos.jld2","RH",CPU1)
FileIO.save("50_CPU2_Menos.jld2","RH",CPU2)
FileIO.save("50_GAP_Menos.jld2","RH",GAP)



#-------------------------------------------------------

Path0 = FileIO.load("50_Path0_Menos.jld2","RH")
Path1 = FileIO.load("50_Path1_Menos.jld2","RH")
Path2 = FileIO.load("50_Path_Menos2.jld2","RH")
Path5 = FileIO.load("50_Path_Menos5.jld2","RH")
Path8 = FileIO.load("50_Path_Menos8.jld2","RH")

Ordem0 = FileIO.load("50_Ordem0_Menos.jld2","RH")
Ordem1 = FileIO.load("50_Ordem1_Menos.jld2","RH")
Ordem2 = FileIO.load("50_Ordem_Menos2.jld2","RH")
Ordem5 = FileIO.load("50_Ordem_Menos5.jld2","RH")
Ordem8 = FileIO.load("50_Ordem_Menos8.jld2","RH")

Visita0 = FileIO.load("50_Visita0_Menos.jld2","RH")
Visita1 = FileIO.load("50_Visita1_Menos.jld2","RH")
Visita2 = FileIO.load("50_Visita2.jld2","RH")
Visita5 = FileIO.load("50_Visita5.jld2","RH")
Visita8 = FileIO.load("50_Visita8.jld2","RH")

Horas0 = FileIO.load("50_Horas0_Menos.jld2","RH")
Horas1 = FileIO.load("50_Horas1_Menos.jld2","RH")
Horas2 = FileIO.load("50_Horas2.jld2","RH")
Horas5 = FileIO.load("50_Horas5.jld2","RH")
Horas8 = FileIO.load("50_Horas8.jld2","RH")

CPU1_Menos = FileIO.load("50_CPU1_Menos.jld2","RH")
CPU2_Menos = FileIO.load("50_CPU2_Menos.jld2","RH")
GAP_Menos = FileIO.load("50_GAP_Menos.jld2","RH")
...
plot(W1,W2,Axes(key = :off,xlabel = :Benefício, ylabel = :Custo),linewidth="2", pointtype="fsquare", plotstyle="linespoints")

#=
p=Plotasolucao(n,L,Path1,atracoes,R,H)
p[end]
